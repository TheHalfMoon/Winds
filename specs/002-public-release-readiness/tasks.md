...
- [x] **T037** Perform a tracked-content public-hygiene audit for secrets/tokens, private or temporary URLs, local absolute paths, accidental personal-machine details, stale internal-only/release language, unsupported security/platform claims, and copied third-party code missing provenance. The corrected read-only probe on exact head `1dbec4ec750b1ae7c0cf61fa9626a64bdbbe8ba6` audited 38 intended tracked files with `HIGH_CONFIDENCE_FINDINGS=0`, `SOURCE_PROVENANCE_MARKERS=0`, and `TRACKED_PUBLIC_HYGIENE_HIGH_CONFIDENCE=PASS`; 47 review-only terms were manually reconciled as explicit non-claims or historical specification truth. `docs/release/public-hygiene-audit.md` records the triage and publication boundary. The temporary self-excluded probe must scan this audit/task-truth snapshot successfully, then be removed; no history rewrite is authorized.
