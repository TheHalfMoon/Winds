...
- when an otherwise unknown T079 notification fails the exact allowlist, rejection diagnostics may record only a static proof phase, the bounded/sanitized protocol method, and sorted/sanitized object-key names for the top-level params and known nested protocol objects; never record raw params, scalar values, IDs, paths, prompt/model text, config values, credential material, or other payload contents, and the diagnostic path must not change the original fail-closed decision;
