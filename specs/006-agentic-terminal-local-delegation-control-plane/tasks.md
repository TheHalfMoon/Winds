...
- when an otherwise unknown T079 notification fails the exact allowlist, rejection diagnostics may record only a static proof phase, static JSON value-shape categories for the method/params and statically named known nested protocol objects, and bounded numeric object-key counts for the top-level params and those known nested objects; never record attacker-controlled method text, object-key text, raw params, scalar values, IDs, paths, prompt/model text, config values, credential material, or other payload contents; compute this metadata only after the protocol client has already failed closed with `UnexpectedT079Notification`, so accepted frames incur no rejection-metadata parse/collection and the diagnostic path cannot change the original fail-closed decision;
