...
- system/managed Codex configuration isolation is a pre-launch fail-closed boundary: existing system configuration surfaces are rejected before any candidate launch rather than silently inherited or read to make the proof pass;
- the connected-proof live platform contract is Linux/WSL2 on seccomp-supported `x86_64` or `aarch64`; unsupported platforms or Linux architectures must short-circuit before any candidate process launch, while the child-side seccomp guard remains defense in depth; descendant process creation/exec denial is part of the bounded proof boundary;
- raw candidate stderr is intentionally discarded with `Stdio::null()` and MUST NOT be captured, copied, persisted, or surfaced because it may contain provider/config/path/credential-sensitive material; diagnostics remain limited to the already-authorized bounded structured protocol and rejection metadata;
...
- disable the Codex 0.149 Stable built-in shell tool for every T079 child with the exact SessionFlags override `features.shell_tool=false`; before any `thread/start`, `config/read` must prove both effective `features.shell_tool=false` and the exact `sessionFlags` origin for `features.shell_tool`. This is a strict authority reduction: it does not admit command/tool activity, does not authorize any replacement tool surface, and does not weaken the existing fail-closed rejection of command execution or terminal interaction;
- disable the Codex 0.149 Stable Apps feature for every T079 child with the exact SessionFlags override `features.apps=false`; before any `thread/start`, `config/read` must prove both effective `features.apps=false` and the exact `sessionFlags` origin for `features.apps`. `include_apps_instructions=false` is a separate instruction-surface reduction and is not evidence that Apps authority is disabled. This Apps reduction must prevent the hosted Apps extension from contributing `codex_apps`; it does not admit MCP activity, does not authorize any replacement MCP/tool surface, and does not weaken the existing fail-closed rejection of MCP/tool/hook notifications;
- the single T079 `turn/start` request must include exactly this fixed `outputSchema`: `{"type":"object","properties":{"status":{"type":"string","const":"WINDS_T079_OK"}},"required":["status"],"additionalProperties":false}`. The only accepted final structured agent message is exactly `{"status":"WINDS_T079_OK"}` with no additional properties; this value is bounded Agent-runtime evidence only and is never verification/acceptance evidence;
- once the exact phase/identity-bound `turn/completed` notification is accepted for the single T079 turn, that turn is terminal: clear its active turn identity and consume any still-pending `TurnStart` request so a delayed `turn/start` response, later `turn/started`, or later item/turn traffic cannot reopen the completed turn; any such delayed or post-terminal frame must fail closed rather than restore active-turn state;
- when a T079 notification has already failed the exact allowlist with `UnexpectedT079Notification`, rejection diagnostics may additionally classify its method into a closed, compile-time-static Codex 0.149 `METHOD_CLASS` and report bounded `KNOWN_KEY_COUNT` / `UNKNOWN_KEY_COUNT` values against that class's statically pinned top-level parameter-key set; known classes are diagnostic labels only, unknown/non-string methods render only as `UNKNOWN_METHOD`, and no class or key count may change admissibility, bind thread/turn identity, mutate request/handshake state, grant authority, or convert rejection into acceptance. Existing static proof-phase and JSON shape categories plus the statically named nested object counts for `thread`, `turn`, `item`, `status`, and `tokenUsage` may remain. All counts saturate at `u16::MAX` (`65535`). Never record attacker-controlled raw method text, unknown object-key text, raw params, scalar values, IDs, paths, prompt/model text, config values, credential material, or other payload contents. Compute all classification/key-count metadata only after the protocol client has already failed closed, so accepted frames incur no rejection-metadata parse/classification and the diagnostic path is non-authoritative. This diagnostic authority does not admit `item/commandExecution/terminalInteraction`, `model/rerouted`, `model/verification`, `model/safetyBuffering/updated`, `turn/moderationMetadata`, `error`, `warning`, `guardianWarning`, wildcard methods, or any other previously inadmissible notification;
...
| `KNOWN_TERMINAL_INTERACTION` | `itemId`, `processId`, `stdin`, `threadId`, `turnId` |
...
This registry is closed for T079: there are exactly 21 known diagnostic classes plus the single `UNKNOWN_METHOD` fallback.

**T079 live-proof diagnostic amendment after the consumed exact-head attempt `1b0e7194b5fa03305c133b2da43cd4ba866e24db`:** the consumed attempt failed closed during the `turn/start` response wait with `METHOD_CLASS=UNKNOWN_METHOD` and `param_key_count=5`. The sanitized receipt intentionally did not retain the raw method or unknown key names, so this amendment does **not** assert which notification was observed. Upstream Codex `rust-v0.149.0` defines `item/commandExecution/terminalInteraction` as an official five-key notification with exactly `itemId`, `processId`, `stdin`, `threadId`, and `turnId`; the upstream event mapping emits it from terminal-interaction command activity. T079 already treats command/tool activity as forbidden. `KNOWN_TERMINAL_INTERACTION` is therefore added only as a post-rejection static diagnostic class so a later separately authorized retry can distinguish this forbidden upstream shape from `UNKNOWN_METHOD`. It remains inadmissible, cannot bind identity or state, cannot grant authority, and cannot convert rejection into acceptance. No second live attempt is authorized by this amendment; implementation of this registry change requires fresh review of this Spec 006 amendment first. `UNKNOWN_METHOD` has no known parameter-key set. Any addition, removal, rename, or change to a class or its exact known top-level key set requires an explicit Spec 006 amendment and fresh review before implementation. Classification and `KNOWN_KEY_COUNT` / `UNKNOWN_KEY_COUNT` computation remain rejection-only and non-authoritative, occur only after `UnexpectedT079Notification` has already failed closed, and cannot alter admissibility, identity binding, request/handshake state, runtime authority, or any other protocol decision. A method being listed above does not make that notification admissible. Counts continue to saturate at `u16::MAX` (`65535`); only the statically named nested object counts `thread`, `turn`, `item`, `status`, and `tokenUsage` may be reported. Raw attacker-controlled method text, unknown key text, raw params, scalar values, IDs, paths, prompt/model text, config values, credential material, and other payload contents remain prohibited from rejection diagnostics.

**T079 authority-hardening amendment after the consumed exact-head attempt `9ba0779fe8c99a9b6831379d8547b542af6d6074`:** the attempt reached the live `run_connected_proof(...)` path and failed closed after the prompt was sent; the locally supplied excerpt proves the canonical ignored test returned `FAILED` after approximately 77.72 seconds and panicked only at the final `.expect("bounded T079 proof")`. The excerpt does not contain the sanitized inner error, so this amendment intentionally does **not** assert that any particular notification, tool call, or terminal interaction caused that failure. Independently, upstream Codex `rust-v0.149.0` defines Stable feature `shell_tool` with `default_enabled: true`, and its exact tool configuration returns `ConfigShellToolType::Disabled` when `Feature::ShellTool` is disabled. Because T079 forbids command/tool activity, the live launch is narrowed by the exact SessionFlags override `features.shell_tool=false`, and `config/read` must prove that reduction before `thread/start`. This amendment does not authorize another live attempt; any retry requires the resulting exact head to pass deterministic qualification and fresh exact-head review, followed by fresh founder authorization tied to that exact SHA.

**T079 Apps-authority amendment after the consumed exact-head attempt `9f3c2c1a43ddfcbd027524d9f1a7fe09e0086651`:** the one authorized attempt on that exact head passed its pre-prompt gates, entered the real connected proof, and was consumed. It failed closed during the `turn/start` response wait with the sanitized rejection metadata `method_shape=STRING`, `METHOD_CLASS=UNKNOWN_METHOD`, `params_shape=OBJECT`, `param_key_count=5`, `KNOWN_KEY_COUNT=0`, and `UNKNOWN_KEY_COUNT=5`; the attempt is consumed and must not be retried. The sanitized transcript intentionally did not retain the raw method or unknown key names, so this amendment does **not** assert which notification was observed. Independently, pinned upstream Codex `rust-v0.149.0` defines Stable `Feature::Apps` with `default_enabled: true`; T079 at `9f3c2c1...` did not disable `features.apps`; and the pinned hosted Apps extension contributes the reserved `codex_apps` MCP server while Apps is enabled and removes it when Apps is disabled. Therefore the material authority defect is repaired narrowly by the exact SessionFlags override `features.apps=false`, with `config/read` required to prove both the effective false value and exact SessionFlags origin before `thread/start`. `include_apps_instructions=false` remains a separate instruction reduction and is not a substitute for disabling Apps authority. No MCP/tool/hook notification becomes admissible, the closed T079 rejection-diagnostic registry is unchanged, this failed attempt is not acceptance evidence, `AUTOMATIC_RETRY=NO`, and any later live attempt requires a new exact head to pass deterministic qualification plus fresh exact-head independent review before separately governed execution authority is evaluated.

**T079 generic-warning suppression amendment after the consumed exact-head attempt `95ebfd2bfa83924e506ad8c4f6a9a0f091907e01`:** the single authorized attempt on that exact head passed its deterministic and review gates, sent the bounded prompt, and then failed closed while waiting in the `turn/start` phase with the sanitized rejection metadata `method_shape=STRING`, `METHOD_CLASS=KNOWN_WARNING`, `params_shape=OBJECT`, `param_key_count=2`, `KNOWN_KEY_COUNT=2`, and `UNKNOWN_KEY_COUNT=0`. The attempt is consumed and MUST NOT be retried. The diagnostic proves only the official Codex `warning` method and its two-key top-level shape; no raw warning message, thread ID, provider data, credential material, path, config value, or other scalar payload is retained or inferred. Pinned upstream Codex `rust-v0.149.0` defines `warning` as the generic `WarningNotification { threadId?: string, message: string }` and routes multiple unrelated informational warning sources through that same method, so admitting `warning` by message or broad shape would widen the T079 protocol surface without a stable semantic boundary. The repair is therefore suppression, not admission: every T079 initialize request MUST set `capabilities.optOutNotificationMethods` to exactly `[`remoteControl/status/changed`, `warning`]` in that order. `warning` remains inadmissible in the T079 parser; if Codex delivers it despite the negotiated opt-out, the existing fail-closed rejection and rejection-only `KNOWN_WARNING` diagnostic remain authoritative. This amendment does not suppress or admit `error`, `guardianWarning`, command/tool/MCP/hook/terminal notifications, model safety/reroute/verification notifications, approval requests, or any other method, and the ordinary non-T079 initialize path remains unchanged. No new live attempt is authorized by this amendment. Implementation requires deterministic tests that prove the exact two-method T079 opt-out and prove the ordinary initialize request has no T079 capability/opt-out surface; afterward the resulting new exact head requires fresh deterministic qualification, fresh independent exact-head review with zero unresolved material findings, and a separately bound fresh one-shot authorization before any new connected proof.

**T079 Codex-0.149 text-input wire-shape amendment after the consumed exact-head attempt `44e6e4ad3c7d0ba9cd6688cd784f0dd28058214e`:** the single authorized host-side attempt on that exact head passed its governed preflight, was consumed, and failed closed in `turn/runtime` with sanitized metadata `method_shape=STRING`, `METHOD_CLASS=KNOWN_ITEM_STARTED`, `params_shape=OBJECT`, `param_key_count=4`, `KNOWN_KEY_COUNT=4`, `UNKNOWN_KEY_COUNT=0`, and `item_key_count=4`. The rejected receipt intentionally does not retain the nested item type, IDs, prompt text, content, paths, provider payload, credentials, or other scalar values, so the item kind is not inferred from the receipt alone. Independently, pinned upstream Codex `rust-v0.149.0` defines App Server v2 `ThreadItem::UserMessage` as a normal `item/started` lifecycle item and defines its `UserInput::Text` wire object with exact keys `type`, `text`, and `text_elements`; the current Winds T079 validator incorrectly requires `textElements`. This is a deterministic compatibility defect inside the already-authorized passive `userMessage` path, not evidence authorizing any new item type or capability. Repair only that wire-key mismatch: T079 text input MUST require `text_elements` and MUST reject `textElements` and every other key drift; it remains text-only, with the exact fixed T079 prompt and bounded `TextElement` validation, and Image, LocalImage, Audio, LocalAudio, Skill, Mention, HookPrompt, CommandExecution, FileChange, MCP, DynamicToolCall, CollabAgentToolCall, SubAgentActivity, WebSearch, ImageView, Sleep, ImageGeneration, review-mode, and all other authority-bearing or non-required item/input surfaces remain inadmissible unless separately amended. Deterministic tests MUST prove the exact Codex 0.149 `text_elements` shape is admitted only within the already-authorized T079 `userMessage`, prove legacy/camelCase `textElements` is rejected, and preserve all existing negative authority/tool/item tests. This amendment does not authorize another live attempt. After implementation, the resulting new exact head must pass deterministic qualification, fresh independent exact-head review with zero unresolved material findings, exact scope/thread reconciliation, and a separately bound fresh one-shot founder authorization before any further connected proof.

- disable Codex App Server remote-control startup for every T079 child with the pinned upstream `CODEX_INTERNAL_APP_SERVER_REMOTE_CONTROL_DISABLED=1` marker so Codex 0.149 selects its ephemeral-disabled startup mode instead of resolving persisted remote-control state; the T079 initialize request must opt out exactly `remoteControl/status/changed` and `warning`, in that order, because Codex 0.149 sends the current remote-control status snapshot to newly initialized clients even when remote control is disabled and T079 intentionally suppresses the heterogeneous generic warning channel rather than admitting its payload. This suppression does not authorize any remote-control request, pairing, persistence, connection, execution, warning payload, or broader notification suppression;
...
**Acceptance evidence**:
- exact executable/version revalidation plus sealed executable-snapshot immutability evidence;
- complete initialize/initialized handshake;
- exact Winds-session <-> native-thread provenance;
- one bounded structured result matching exactly `{"status":"WINDS_T079_OK"}` under the pinned T079 `outputSchema`;
