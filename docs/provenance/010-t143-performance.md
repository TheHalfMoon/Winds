...
Four measured RSS failures now separately justify the narrow dependency allowance contemplated by T143 acceptance. The next successor exposes the exact already-locked transitive `webkit2gtk = 2.0.2` crate as a Linux-only direct host dependency without introducing any new locked third-party package or version. It applies WebKitGTK's `DocumentViewer` cache model to the existing Winds WebContext after creation. This is a product setting for a single local application surface, not benchmark-only instrumentation; WebKitGTK documents the document-viewer model as the minimum-cache mode intended to substantially reduce memory for single-document/local-viewer applications. The normal security, accessibility, native-platform, terminal, and renderer interaction gates remain unchanged. The benchmark browser is not treated as native RSS authority; native Tauri process-tree sampling remains the sole RSS qualification source.
