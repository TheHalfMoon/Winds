...
**Herdr exact pin:** `herdrdev/herdr@18061191fdc019498610aee81f0df93f6c2ebd31`
...
The ledger is source-derived from four surfaces at the exact pin: (1) the complete 104-method public socket API enum, (2) all 24 compiled agent-detection families, (3) all 17 compiled installable integration targets, and (4) feature-bearing runtime/client/config/plugin/test modules that expose product behavior beyond the public API. A fresh exact-pin reconciliation is required before each successor Spec entry and again before final closeout.
...
`HERDR_FULL_PARITY=PROVEN` is forbidden until this ledger has no `Required` row without evidence-backed final disposition, the current Herdr source has been re-audited for feature drift, all admitted direct/adapted copies have provenance/license ledgers, and Spec 015 parity closeout reconciles current Winds behavior against both this pin and any newer Founder-accepted Herdr pin.
